...
class VisionModel2(nn.Module):
    ...
    def forward(self, x):
        ...
        return {"feature":output['reshaped_hidden_states'], "project":project}
...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.swinViT2 = VisionModel2(vision_type2, project_dim)
    ...
    def forward(self, data):
        ...
        if x_in.shape[1] == 1:   
            x_in = repeat(x_in,'b 1 h w -> b c h w',c=3)

        if not torch.jit.is_scripting():
            self._check_input_size(x_in.shape[2:])
        image_output = self.swinViT(x_in)
        image_output2 = self.swinViT2(x_in)
        hidden_states_out, image_project = image_output['feature'], image_output['project']
        hidden_states_out2, image_project2 = image_output2['feature'], image_output2['project']
        ...
        dec4 = self.encoder10(hidden_states_out2[3])
        #print(f"shape of dec4 is : {dec4.shape}")
        dec3 = self.decoder384(rearrange(dec4,'b c h w -> b (h w) c'), rearrange(hidden_states_out2[2],'b c h w -> b (h w) c'),text_embeds[-1])
